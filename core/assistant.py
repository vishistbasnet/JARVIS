...
class Assistant:
    ...
    def process_once(
        ...
        gemini_response, function_call = self.tool_caller.request_tool_call(user_text)
        if function_call is not None:
            logger.info("Gemini selected tool: %s", function_call.name)
            tool_result = self.tool_caller.execute_tool_call(function_call)
            ...
            logger.info("No tool required. Using normal LLM response.")
            response = self.llm.chat(user_text, history=history[:-1])
